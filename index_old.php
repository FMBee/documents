					...
					<?php foreach(getDir(BASE) as $element) { ?>
						<li>
							<a href="index.php?p=<?php echo rawurlencode(BASE . "/" . $element['nom']) ?>">
								...
								<span <?php echo rawurlencode(BASE . "/" . $element['nom']) == rawurlencode($repertoire_courant) ? 'style="font-weight:bold"' : '' ?> title="<?php echo normalizeString($element['nom']) ?>"><?php echo shortenString(normalizeString($element['nom']), 15) ?></span>
							</a>
						</li>
					<?php } ?>
				</ul>
									...
									case 'fichier':
										if( in_array(strtolower($element['extension']), $t_extensions_reconnues) ) { 
											...
											<td class="element2_1 fichier" id="element_<?php echo rawurlencode($repertoire_courant."/".$element['nom.extension']), "&amp;orderby=nom&amp;order=", $order ?>" title="<?php echo $element['nom.extension'] ?>">
												...
												<a href="<?php echo $repertoire_courant."/".$element['nom.extension'];?>"><?php echo shortenString(normalizeString($element['nom.extension']), 44) ?></a>
											</td>
